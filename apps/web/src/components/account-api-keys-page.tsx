...
function formatModelList(value: string[]) {
	return value.join(", ");
}
...
export function AccountApiKeysPage() {
	...
	const byokProvidersQuery = useQuery(trpc.apiKeys.byok.providers.queryOptions());
	...
	return (
		<main className="flex flex-col gap-6 pb-28">
			<div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
				<div className="space-y-1">
					...
					<p className="max-w-3xl text-muted-foreground text-sm">
						Create GitPal API keys for app access, then manage bring-your-own-provider
						keys that can route model calls directly and bypass wallet spend.
					</p>
				</div>
							...
							<CardContent className="space-y-4">
								...
								<div className="space-y-2">
									...
									<Select
										...
										onValueChange={(value) => setAppKeyExpiration(value ?? "90")}
									>
								...
								<Button
									type="button"
									disabled={createAppKeyMutation.isPending || !appKeyName.trim()}
									onClick={() =>
								...
								>
									{createAppKeyMutation.isPending ? "Creating..." : "Create key"}
								</Button>
								{latestCreatedKey ? (
									<div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4">
										...
										<code className="mt-3 block rounded-xl bg-background px-3 py-2 text-sm break-all whitespace-normal">
											{latestCreatedKey}
										</code>
									</div>
								) : null}
								...
								{appKeys.length ? (
												...
												<div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
													<div className="min-w-0 space-y-3">
														...
														<div className="flex flex-wrap items-center gap-2 text-sm">
															<code className="inline-flex max-w-full break-all rounded-lg bg-muted px-2 py-1 text-xs whitespace-normal">
																{[key.prefix, key.start].filter(Boolean).join("") ||
																	"Protected"}
															</code>
															<Badge
									...
									<Empty className="min-h-72">
										<EmptyHeader>
											...
											<EmptyDescription>
												Create a GitPal API key for scripts, automations, or external integrations.
											</EmptyDescription>
										</EmptyHeader>
									</Empty>
								)}
					...
					<div className="grid gap-6 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Prefer your own provider keys first, then fall back to a router when needed.
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								<div className="space-y-2">
								...
								<div className="flex items-center justify-between rounded-2xl border border-border/60 bg-muted/20 px-4 py-3">
									<div>
										<div className="font-medium text-sm">Prefer your own keys</div>
										<p className="text-muted-foreground text-sm">
											When a provider key exists, GitPal will route there first so the call is free for you.
										</p>
									</div>
									<Switch
							...
							<CardContent className="space-y-4">
								<div className="flex flex-col gap-3 md:flex-row">
									<Input
										...
										onChange={(event) => setRoutePreviewModel(event.target.value)}
								...
								{previewRouteQuery.data ? (
									<div className="rounded-2xl border border-border/60 bg-muted/20 p-4">
										<div className="flex items-center gap-2">
											...
											<div className="font-medium">{previewRouteQuery.data.label}</div>
										</div>
					...
					<div className="grid gap-6 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Choose a provider, then store one or more keys in priority order.
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								...
								<div className="space-y-2">
									{filteredProviders.map((provider) => {
										...
										<button
												key={provider.id}
												type="button"
												className="flex w-full items-center justify-between gap-3 rounded-2xl border border-border/60 bg-card/70 px-4 py-3 text-left transition-colors hover:bg-muted/30"
											onClick={() => {
						...
						<div className="space-y-6">
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Store encrypted provider secrets and define how they should match models.
									</CardDescription>
								</CardHeader>
								<CardContent className="space-y-4">
									<div className="space-y-2">
										...
										<Select
											value={providerId}
											onValueChange={(value) =>
												setProviderId((value as LlmProviderId) ?? providerId)
											}
										>
											<SelectTrigger className="w-full">
									...
									<div className="space-y-2">
										<div className="font-medium text-sm">
											API key {editingKeyId ? "(leave blank to keep existing)" : ""}
										</div>
										<Input
									...
									<div className="space-y-2">
										...
										<Textarea
											...
											onChange={(event) =>
												setProviderAllowedModels(event.target.value)
											}
											className="min-h-24"
											placeholder="Leave empty for all models from this provider, or list exact prefixes like google/gemini-2.5-pro"
										/>
									</div>
									<div className="grid gap-3 md:grid-cols-2">
										...
										<div className="flex items-center justify-between rounded-2xl border border-border/60 bg-muted/20 px-4 py-3">
											<div>
												...
												<p className="text-muted-foreground text-xs">
													Do not prefer router-based fallbacks for matching models.
												</p>
											</div>
											<Switch
										...
										<Button
											...
											onClick={() =>
												saveByokKeyMutation.mutate({
													...
													allowedModels: parseModelList(providerAllowedModels),
												})
											}
										>
											{saveByokKeyMutation.isPending
							...
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Enabled direct keys bypass wallet billing when they match the requested model.
									</CardDescription>
								</CardHeader>
								<CardContent>
									{byokKeys.length ? (
													...
													<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
														<div className="min-w-0">
															<div className="flex flex-wrap items-center gap-2">
																...
																<Badge variant="secondary">{key.providerLabel}</Badge>
																<Badge variant="outline">
														...
														<div className="flex flex-wrap items-center gap-2">
															...
															<Button
																...
																onClick={() => {
																	...
																	setProviderAllowedModels(
																		formatModelList(key.allowedModels),
																	);
																}}
															>
																Edit
										...
										<Empty className="min-h-72">
											<EmptyHeader>
												...
												<EmptyDescription>
													Add a provider key to route compatible models directly and avoid wallet spend for those calls.
												</EmptyDescription>
											</EmptyHeader>
										</Empty>
									)}
			...
			<SettingsChangeDock
				...
				saveLabel={updateRoutingMutation.isPending ? "Saving..." : "Save routing"}
