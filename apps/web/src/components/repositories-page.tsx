...
export function RepositoriesPage() {
  ...
  const pageSizeOptions = React.useMemo(
    () =>
      ["10", "25", "50", "100"].map((size) => ({ label: size, value: size })),
    [],
  );
  ...
  const syncMutation = useMutation(
    trpc.repositories.sync.mutationOptions({
      onSuccess: async (result) => {
        ...
        if (!activeWorkspaceId && result.workspaceIds[0]) {
          await switchWorkspace(result.workspaceIds[0]);
        }

        toast.success(
          `Repository sync completed. Webhooks: ${result.webhooks.created} created, ${result.webhooks.existing} connected, ${result.webhooks.failed} failed.`,
        );
      },
      onError: (error) => {
  ...
  const syncWebhooksMutation = useMutation(
    trpc.repositories.syncWebhooks.mutationOptions({
      onSuccess: (result) => {
        toast.success(
          `Webhook sync finished. ${result.created} created, ${result.existing} connected, ${result.failed} failed.`,
        );
      },
      onError: (error) => {
        toast.error(error.message);
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            synced as a {formatScope(activeWorkspace.scope).toLowerCase()}{" "}
            workspace from {activeWorkspace.providerName}.
          </CardDescription>
          <CardAction>
            <div className="flex flex-wrap gap-2">
              <Badge variant="outline">
                {formatScope(activeWorkspace.scope)}
              </Badge>
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex min-w-0 items-center gap-3">
                        <div className="flex size-10 shrink-0 items-center justify-center rounded-lg border bg-muted/60 text-xs font-medium">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <Badge
                        variant={
                          repository.webhookConnected ? "secondary" : "outline"
                        }
                      >
                        {repository.webhookConnected
                          ? repository.webhookLastDeliveredAt
                            ? `Webhook active · ${formatDistanceToNow(
