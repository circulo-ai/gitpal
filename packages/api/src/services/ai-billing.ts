...
function toUsage(value: unknown): AiCallUsage {
	...
	return {
		...
		outputReasoningTokens:
			...
			toNumber((usage as { reasoningTokens?: unknown } | null)?.reasoningTokens) ??
			0,
...
function getModelProviderId(modelId: string, routePreview: ModelRoutePreview) {
	return (
		inferProviderIdFromModel(modelId) ??
...
export async function runTrackedAiGeneration<TResult>({
	...
	try {
		...
		await failGenerationRow({
			generationId,
			errorMessage: error instanceof Error ? error.message : "ai_generation_failed",
			callKind,
			modelId,
			routePreview,
			tags,
