...
export async function runRepositoryLabeler(context: LabelerContext) {
	...
	const repositoryLabels =
		...
		(await context.adapter.listRepositoryLabels({
	...
	if (repositoryLabels.length === 0) {
		return {
			summary: "No repository labels exist yet, so no labeler suggestions were generated.",
			suggestedLabels: [],
			appliedLabels: [],
