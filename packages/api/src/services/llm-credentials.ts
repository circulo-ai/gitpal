...
export async function previewModelRouteForUser({
	...
	const routerIds = [
		settings.defaultRouter,
		settings.fallbackRouter,
	].filter((routerId): routerId is LlmRouterId => Boolean(routerId));
	...
	for (const [index, routerId] of routerIds.entries()) {
		...
		const providerId = routerId === "openrouter" ? "openrouter" : "openai";
