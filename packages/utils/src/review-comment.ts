...
function isVisiblePath(path: string, filters: string[]) {
	...
	return !filters.some((pattern) => path.includes(pattern.replace(/\*\*/g, "")));
}
...
function buildSuggestedReviewersSection(reviewers: string[]) {
	...
	return reviewers.map((reviewer) => `- ${escapeMarkdown(reviewer)}`).join("\n");
}
...
function buildToolStatusLabel(tool: WorkspaceManagedTool) {
	...
	return tool.mcpServerName ? `MCP · ${tool.mcpServerName}` : "MCP · missing server";
}
...
function buildPreviewNotes(settings: WorkspaceSettings, toolRows: WorkspaceToolPreviewRow[]) {
...
function buildPreviewFindings(settings: WorkspaceSettings): ReviewCommentFinding[] {
			...
			return [
				{
					...
					category: "security",
					body:
						"Mark the token as consumed before returning the success response so retries cannot reuse it.",
			...
			return [
				{
					...
					body:
						"Cache the repository row for the request instead of re-querying after the validation pass.",
			...
			return [
				{
					...
					body:
						"Extract the tool-policy merge into a helper so the workspace and repository paths stay aligned.",
			...
			return [
				{
					...
					body:
						"Guard the comment publication path so a missing author or empty body does not silently short-circuit the review.",
	...
	return [
		{
			...
			status: "passed",
			details: "The pull request description is present and gives reviewers enough context.",
		},
		{
			name: "Docstring coverage",
			status: settings.finishingTouches.docstrings.enabled ? "passed" : "warning",
...
export function buildRepositoryReviewCommentData(
	...
	const suggestedLabels =
		input.settings.reviews.walkthrough.suggestedLabels && input.suggestedLabels
			...
			: [];
	const reviewEffort = input.settings.reviews.walkthrough.estimateCodeReviewEffort
		? input.reviewEffort ?? estimateReviewEffort(visibleFiles, findings)
		: null;
	const suggestedReviewers =
		input.suggestedReviewers && input.suggestedReviewers.length > 0
...
export function buildRepositoryReviewPreviewData(
	...
	return buildRepositoryReviewCommentData(buildPreviewInput(settings, overrides));
}
